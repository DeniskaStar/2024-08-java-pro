...
//        when(accountDao.findById(30L)).thenReturn(Optional.of(destinationAccount));
    ...
    void testTransferWithCommission() {
        ...
        BigDecimal commissionPercent = new BigDecimal("0.1");

        doReturn(List.of(sourceAccount))
                .when(accountService).getAccounts(argThat(agreement -> agreement != null && agreement.getId().equals(1L)));

        doReturn(List.of(destinationAccount))
                .when(accountService).getAccounts(argThat(agreement -> agreement != null && agreement.getId().equals(2L)));
        ...
        paymentProcessor.makeTransferWithComission(sourceAgreement, destinationAgreement,
                0, 0, BigDecimal.ONE, commissionPercent);

        // Then
        assertEquals(new BigDecimal("9.1"), sourceAccount.getAmount());
        assertEquals(BigDecimal.ONE, destinationAccount.getAmount());

        verify(accountService).charge(sourceAccount.getId(), new BigDecimal("-0.1"));
    }
}
